...
- [ ] Residuals rolled into hardening: DIE packer / compiler / crypto fingerprinting, MACO config extraction, restart-surviving L1 graph persistence
...
**Phase 4 - Reporting, interop, and API** &nbsp;`PLANNED` - STIX 2.1 / MISP export, a copyable IOC panel, a stable public REST API + OpenAPI, webhooks, batch submission, and SIEM/TheHive connectors.

**Phase 5 - Intelligence and learning at scale** &nbsp;`PLANNED` - L1 graph persistence, a curated-corpus growth pipeline with trust-tiered OSINT ingestion, learning tiers 2 and 3 (DSPy prompt-opt, LoRA fine-tune) as offline eval-gated jobs, and drift monitoring.
