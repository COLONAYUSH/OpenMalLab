...
| **Provider** | [`internal/aiplane/httpprovider.go`](../internal/aiplane/httpprovider.go) | `NewLocalProvider` (the only one the orchestrator wires, [`main.go`](../services/mal-orchestrator/main.go)) enforces a sovereign loopback/private/container host via `requireSovereignHost`; `NewCloudProvider` is a defined-but-unwired explicit egress escape hatch. The roster's Python provider mirrors this posture and gates non-sovereign egress behind `MAL_ALLOW_CLOUD=1` ([`services/mal-agents/malagents/provider.py`](../services/mal-agents/malagents/provider.py)). | Live |
...
| **L1** relational graph | [`graph.go`](../internal/knowledge/graph.go), [`persist_graph.go`](../internal/knowledge/persist_graph.go) | Bounded BFS attribution graph; `PathCurated` from a curated-only walk | Live in-memory (`MemGraph`); a restart-surviving BoltDB graph store (`BoltGraph`, atomic + poisoning-guarded, `OpenGraphFromEnv`) is built and on `main`, not yet wired into orchestrator startup |
...
## 8. Reporting and export (built, API not yet wired)

[`internal/export/`](../internal/export/) - deterministic STIX 2.1 bundle + MISP event
export and defanged IOC extraction, computed purely from the verdict contract:
[`export.go`](../internal/export/export.go) (the surface),
[`stix.go`](../internal/export/stix.go), [`misp.go`](../internal/export/misp.go), and
[`iocs.go`](../internal/export/iocs.go). Pure functions over `internal/pipeline` output,
so the same verdict always yields the same bundle and nothing here re-parses hostile
bytes. Built and unit-tested on `main`; the `GET /v1/submissions/{id}/export` gateway
route that would expose it is the next wiring step (Phase 4). Status: built, API not yet
wired.
...
## 10. Where a new contributor should start
